    ...
    <div className="m-sm-30">
      <div className="mb-sm-30">
        ...
        <Grid container spacing={2}>
          ...
          <Grid item lg={3} md={6} xs={12}>
          <span>Total Tax Amount</span>:    <strong>{parseFloat(net_tax_amount).toLocaleString(undefined, {minimumFractionDigits:2})}</strong>
            </Grid>
        </Grid>
      </ValidatorForm>
       ...
       data={sales_Report.map((item, index) => {
        ...
        return [
         ++index,
          ...
          item.party_id?item.quotation.party['firm_name']:'',
          item.quotation.party?.vat_no,
          item.invoice_no,
          item.total_value,
          item.vat_in_value
          // (parseFloat(parseFloat(item.net_amount)+(parseFloat(item.net_amount)*0.15))).toLocaleString(undefined, {maximumFractionDigits:2}) 
       ]
